...
QWidget* HistogramWidget::CreateTitleWidget() {
...
QWidget* HistogramWidget::CreateControlWidget() {
...
QWidget* HistogramWidget::CreateSwitchWidget() {
...
void HistogramWidget::SlotOpenFile() {
    QString file_name = QFileDialog::getOpenFileName(this, "选择图片", "D:/Pictures", "image files(*.png *.jpeg *.jpg)");
...
void HistogramWidget::ClearList() {
    // 清理list
    list_items.clear();
    delete model;
...
void HistogramWidget::SlotDehaze() {
    if (dark != NULL) {
        this->ClearList();
        //dark->SetArgs(is_guide, is_sky, min_filter_R, guide_filter_R, sky_K, clahe_L, clahe_S);
...
void HistogramWidget::UpdateListView(ImgStruct img_struct, QString name) {
    QImage img = Tools::ConverMatToImage(img_struct.mat);
...
void HistogramWidget::SlotShowImage(QImage image) {
     emit SignalShowImage(image);
}

void HistogramWidget::SlotSetControlLabelText(int value) {
...
void HistogramWidget::SlotSwitchGuide(bool is_open) {
    is_guide = is_open;
}

void HistogramWidget::SlotSwitchSky(bool is_open) {
